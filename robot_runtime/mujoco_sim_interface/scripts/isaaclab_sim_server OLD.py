...
class IsaacLabSimServer:
    ...
    def _create_scene(self):
        ...
        n_total = self.robot.num_joints
        self._nt = n_total

        self._act_cpu_t = torch.zeros(5 * n_total, dtype=torch.float32, pin_memory=True)
        self._act_cpu = self._act_cpu_t.numpy()
        self._act_gpu = torch.zeros(5 * n_total, device=self.device)
        self._q_des_full = self._act_gpu[0*n_total:1*n_total]
        self._qd_des_full = self._act_gpu[1*n_total:2*n_total]
        self._kp_full = self._act_gpu[2*n_total:3*n_total]
        self._kd_full = self._act_gpu[3*n_total:4*n_total]
        self._ff_full = self._act_gpu[4*n_total:5*n_total]

        self._err_q = torch.zeros(n_total, device=self.device)
        self._err_qd = torch.zeros(n_total, device=self.device)
        self._tau_1d = torch.zeros(n_total, device=self.device)
        self._tau_2d = self._tau_1d.unsqueeze(0)
        self._snap_size = 7 + 6 + n_total + n_total
        self._snap_gpu = torch.zeros(self._snap_size, device=self.device)
        self._snap_cpu = torch.empty(self._snap_size, dtype=torch.float32, pin_memory=True)
        self._snap_jpos_start = 13
        self._snap_jvel_start = 13 + n_total
        self._snap_step = 0
        self._snap_interval = 1
    ...
    def _simulation_step(self):
        ...
        with self.data_lock:
            ...
            self.reset_requested = False
            self._act_cpu[idx] = self.latest_q_des
            self._act_cpu[nt + idx] = self.latest_qd_des
            self._act_cpu[2*nt + idx] = self.latest_kp
            self._act_cpu[3*nt + idx] = self.latest_kd
            self._act_cpu[4*nt + idx] = self.latest_ff

        self._act_gpu.copy_(self._act_cpu_t)
        ...
        self._phase_lock += 1
        self._phase_pd += (t2 - t1)
        self._phase_sim += (t3 - t2)
        self._phase_snap += (t4 - t3)
        self._phase_total += (t4 - t0)
        if self._phase_lock % 2000 == 0:
            m = 2000
            logger.info(
                "[profile] lock=%.2fms  pd=%.2fms  sim=%.2fms  snap=%.2fms  total=%.2fms",
                (t1 - t0) * 1e3,
                self._phase_pd / m * 1e3,
                self._phase_sim / m * 1e3,
                self._phase_snap / m * 1e3,
                self._phase_total / m * 1e3,
            )
            self._phase_pd = self._phase_sim = self._phase_snap = self._phase_total = 0.0

        if root_z is not None and root_z < 0.2:
